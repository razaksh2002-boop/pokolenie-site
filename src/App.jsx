...
const MobileStyles=()=><style>{`*{box-sizing:border-box;}input,textarea,select{font-size:16px!important;}@media(max-width:600px){.hero-btns{flex-direction:column!important;}.hero-btns>*{width:100%!important;text-align:center;}.founder-grid{grid-template-columns:1fr!important;}.contact-grid{grid-template-columns:1fr!important;}.stats-grid{grid-template-columns:1fr 1fr!important;}.modules-grid .mod-item:not(:last-child){border-right:none!important;border-bottom:1px solid rgba(168,131,42,.12)!important;}}`}</style>;
    ...
    <Rv delay={.2}>
      <div style={{background:NAVY_MID,border:`1px solid rgba(168,131,42,.18)`,borderRadius:8,padding:"28px 20px"}}>
        <svg viewBox="0 0 1000 480" style={{width:"100%",height:"auto",display:"block"}}>
          ...
          {/* Sub-Saharan Africa partial OIC */}
...
function Contact({t}){
  ...
  return(<Sec id="contact" bg={NAVY_MID}><GP opacity={.05}/><Wrap>
    ...
    <div className="contact-grid" style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:36}}>
      ...
      <Rv delay={.2}>
        <div className="contact-grid" style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:8,marginBottom:8}}>
